...
export default function PetProfilePage() {
  ...
  const handleInteresse = async () => {
    if (!usuario) return alert("Entre para demonstrar interesse.");
    await addInteresse(pet.id, usuario.nomeCompleto);
  ...
  const handleAvaliar = async () => {
    if (!usuario || !pet) return alert("Entre para avaliar.");
    await addAvaliacao(pet.id, { usuario: usuario.nomeCompleto, nota, comentario: comentario || undefined });
      ...
      <div className="bg-white/10 p-6 rounded-xl shadow-2xl border border-white/10">
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="lg:w-1/3">
            <img src={pet.fotoUrl} alt={pet.nome} className="w-full h-96 object-cover rounded-lg shadow-lg border-4 border-primary/50" />
          </div>
          <div className="lg:w-2/3">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <button onClick={handleInteresse} className="flex items-center gap-2 px-4 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700">
                <HeartIcon className="h-5 w-5" /> Tenho Interesse
              </button>
            </div>
            ...
            <div className="mt-8 bg-white/5 border border-white/10 rounded-lg p-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <label htmlFor="nota-select" className="sr-only">Nota</label>
                <select
                  id="nota-select"
                  value={nota}
                  onChange={e => setNota(Number(e.target.value))}
                  className="bg-black/30 text-gray-100 border border-white/10 rounded px-2 py-1"
                  aria-label="Nota"
                >
                  {[1, 2, 3, 4, 5].map(n => <option key={n} value={n}>{n} ★</option>)}
                </select>
              </div>
              <label htmlFor="comentario-avaliacao" className="sr-only">Comentário</label>
              <textarea
                id="comentario-avaliacao"
                value={comentario}
                onChange={e => setComentario(e.target.value)}
                className="w-full mt-3 bg-black/30 text-gray-100 border border-white/10 rounded p-2"
                placeholder="Comentário (opcional)"
                aria-label="Comentário da avaliação"
              />
              <button onClick={handleAvaliar} className="mt-3 flex items-center gap-2 px-4 py-2 bg-secondary text-black rounded-lg hover:bg-secondary/80">
                <StarIcon className="h-5 w-5" /> Enviar avaliação
              </button>
            </div>
          </div>
        </div>
